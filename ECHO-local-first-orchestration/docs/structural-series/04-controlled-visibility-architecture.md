...
Modern AI systems assume that more visibility leads to better performance. But in local-first environments - where models operate beside private files, business records, creative drafts, and operational data - unbounded visibility becomes a liability. Controlled Visibility Architecture (CVA) is an architectural pattern that governs what the model can see, when it can see it, and under what contextual constraints. CVA enforces reversible, corridor-bound, state-aware perception. A hypothetical case study - Echoleak, a zero-click perception-boundary failure - is used to illustrate why visibility must be governed before cognition, not after. This paper defines CVA, explains its relationship to the Perception Firewall and Trust-Zone Routing, and outlines how it prevents over-perception and cognitive drift in sovereign local-first AI systems.

## 1. Introduction

Local AI systems operate in proximity to sensitive information:
...
- operational logs
- private media
This proximity creates a structural tension:
...
Controlled Visibility Architecture resolves this tension by treating perception as a governed resource, not an open stream.

The core principle:

Visibility must be intentional, contextual, reversible, and corridor-bound.

## 2. The Problem: Unrestricted Perception

When a model sees too much, several predictable failure modes emerge.
...
The Echoleak case study demonstrates that ungoverned visibility is the root cause of many AI safety failures.
...
- Can be governed
- Can be reversed
- Is exactly what CVA addresses
CVA governs runtime visibility, not training-time contamination.

This distinction closes the conceptual gap that often confuses discussions of model leakage.

## 4. Controlled Visibility Architecture

CVA governs how perception is exposed to the model.

It is built on three pillars:

- Contextual Visibility
- Reversible Visibility
- Corridor-Bound Visibility
These pillars ensure that perception is not a firehose - it is a controlled, state-aware stream.

## 5. The Visibility Stack

CVA defines a five-layer visibility stack that mirrors the ECHO cognitive architecture.

### Layer 1 - Doctrine (Why visibility is governed)

### Layer 2 - Interpreter (How visibility is governed)

### Layer 3 - Corridors (When visibility is allowed)

### Layer 4 - Execution (What visibility is exposed)

### Layer 5 - Reality (Where visibility originates)

Each layer plays a distinct role.

## Layer 1 - Doctrine

Defines the philosophical boundaries:
...
- exposure limits
- contextual gating
- state-aware perception
Interpreter answers:

How is visibility governed?
...
Where does visibility originate?

## 6. The Visibility Components (Upgraded)

Earlier drafts described these components aspirationally.
...
CVA assumes hostile inputs, not good-faith ones.
...
Controlled Visibility Architecture reframes perception as a governed resource rather than an unbounded input stream. By enforcing contextual, reversible, corridor-bound visibility, local-first AI systems become:
...
- more aligned
- more sovereign
This paper defines CVA as a standalone architectural primitive.
